...
public class OVRPlayerController : MonoBehaviour
{
	...
	private bool RThumbstickPressed = false;
	private bool LThumbstickPressed = false;
	...
	void Update()
	{
		if(OVRInput.Get(OVRInput.RawButton.RThumbstick, OVRInput.Controller.RTouch)){
			if(MoveScale < 10 && !RThumbstickPressed){
				MoveScale +=1;
				Debug.Log(MoveScale);
			}
			RThumbstickPressed = true;
		}else{
			RThumbstickPressed = false;
		}

		if(OVRInput.Get(OVRInput.RawButton.LThumbstick, OVRInput.Controller.LTouch)){
			if(MoveScale > 1 && !LThumbstickPressed){
				MoveScale -=1;
				Debug.Log(MoveScale);
			}
			LThumbstickPressed = true;
		}else{
			LThumbstickPressed = false;
		}
	...
	public virtual void UpdateMovement()
	{
		...
		if (EnableLinearMovement)
		{
			...
			bool moveUp = false;
			bool moveDown = false;
			if(moveUpAndDownEnabled){
				moveUp = OVRInput.Get(OVRInput.RawButton.RThumbstickUp, OVRInput.Controller.RTouch);
				moveDown = OVRInput.Get(OVRInput.RawButton.RThumbstickDown, OVRInput.Controller.RTouch);
			}
			...
			float TempMoveScale = MoveScale;

			if ((moveForward && moveLeft) || (moveForward && moveRight) ||
				(moveBack && moveLeft) || (moveBack && moveRight))
				TempMoveScale = TempMoveScale/0.70710678f;

			// No positional movement if we are in the air
			//lines commented out by luca
			//if (!Controller.isGrounded)
			//	MoveScale = 0.0f;

			TempMoveScale *= SimulationRate * Time.deltaTime;

			// Compute this for key movement
			float moveInfluence = Acceleration * 0.1f * TempMoveScale * MoveScaleMultiplier;
			...
			moveInfluence = Acceleration * 0.1f * TempMoveScale * MoveScaleMultiplier;

...
public class LayersMovementScript : MonoBehaviour
{
    ...
    public Mesh[] decimatedLayersMeshes;
    public Mesh[] originalLayersMeshes;

    private bool isLowquality = true;
    ...
    //also checks for model scaleto change mlayer's meshes definition when threshold is passed
    private void Update()
    {
        if(isActive){
        ...
        if(isLowquality && transform.parent.localScale.x > 1.5f){
            isLowquality = false;
            for(int i = 0; i<5; i++){
                transform.GetChild(i).GetChild(0).GetComponent<MeshFilter>().mesh = originalLayersMeshes[i];
            }
        }
        if(!isLowquality && transform.parent.localScale.x < 1.5f){
            isLowquality = true;
            for(int i = 0; i<5; i++){
                transform.GetChild(i).GetChild(0).GetComponent<MeshFilter>().mesh = decimatedLayersMeshes[i];
            }
        }
    }
